...
static void debug_queue_add(struct list_head *root, struct queue_message *qm) {
  ...
  struct queue_message *old =
      list_first_entry(root, struct queue_message, node);
  list_del(&old->node);
  message_free(old);
}

// Wake up the receiver thread if it is waiting  如果正在等待，唤醒接收线程
static void check_wake_receive(struct serialqueue *sq) {
...
static double calculate_bittime(struct serialqueue *sq, uint32_t bytes) {
...
// 当接收序列增加时更新内部状态
static void update_receive_seq(struct serialqueue *sq, double eventtime,
                               uint64_t rseq) {
  // Remove from sent queue  从发送队列中移除
  uint64_t sent_seq = sq->receive_seq;
  for (;;) {
    struct queue_message *sent =
        list_first_entry(&sq->sent_queue, struct queue_message, node);
...
#endif
    sent_seq++;
    if (rseq == sent_seq) { // Found sent message corresponding with the
                            // received sequence  找到与接收序列对应的发送消息
...
static void handle_message(struct serialqueue *sq, double eventtime, int len) {
  ...
  // Check for pending messages on notify_queue  检查 notify_queue
  // 上的待处理消息
  int must_wake = 0;
  while (!list_empty(&sq->notify_queue)) {
    struct queue_message *qm =
        list_first_entry(&sq->notify_queue, struct queue_message, node);
  ...
  if (len == MESSAGE_MIN) {
    ...
    qm->sent_time =
        (rseq > sq->retransmit_seq ? sq->last_receive_sent_time : 0.);
  ...
  list_for_each_entry(fr, &sq->fast_readers, node) {
    if (len < fr->prefix_len + MESSAGE_MIN ||
        memcmp(&sq->input_buf[MESSAGE_HEADER_SIZE], fr->prefix,
               fr->prefix_len) != 0)
      continue;
...
static void input_event(struct serialqueue *sq,
                        double eventtime) //---11-3task-G-G--RW_task-
{
  // LOG_D("input_event\n");
  if (sq->serial_fd_type == SQT_RPBUF) {
    int ret = combus_read(sq->bus, &sq->input_buf[sq->input_pos],
  ...
  } else if (sq->serial_fd_type == SQT_MEM) { //---G-G-2022-
    if (msgbox_read_signal(MSGBOX_ALLOW_READ)) {
      int ret = 0;
      // ret = do_read_sq(&sq->input_buf[sq->input_pos], sizeof(sq->input_buf) -
      // sq->input_pos); //---12-3task-G-G--RW_task-
      ret = combus_read(sq->bus, &sq->input_buf[sq->input_pos],
                        sizeof(sq->input_buf) - sq->input_pos);
      if (ret > 0) { //---gg-6.22
        sq->input_pos += ret;
      }
...
// 用于管道FD上的输入活动的回调
static void kick_event(struct serialqueue *sq,
                       double eventtime) //---12-3task-G-G--RW_task-
{
  char dummy[4096];
...
static double retransmit_event(
    struct serialqueue *sq,
    double
        eventtime) //---5-3task-G-G--RW_task-//---send-G-G-2022-07-22---SQPT_RETRANSMIT
{
...
build_and_send_command(struct serialqueue *sq, uint8_t *buf, int pending,
                       double eventtime) //----2-G-G-2022-04-08-----------------
{
  ...
  while (sq->ready_bytes) {
    ...
    list_for_each_entry(q, &sq->pending_queues, node) {
      if (!list_empty(&q->ready_queue)) {
        struct queue_message *m =
            list_first_entry(&q->ready_queue, struct queue_message, node);
    ...
    if (qm->notify_id) {
      // Message requires notification - add to notify list  消息需要通知 -
      // 添加到通知列表
...
// 确定应发送下一个串行数据的时间
static double check_send_command(struct serialqueue *sq, int pending,
                                 double eventtime) {
  if (sq->send_seq - sq->receive_seq >= sq->max_pending_blocks &&
  ...
  list_for_each_entry(cq, &sq->pending_queues, node) {
    ...
    while (!list_empty(&cq->stalled_queue)) {
      struct queue_message *qm =
          list_first_entry(&cq->stalled_queue, struct queue_message, node);
    ...
    if (!list_empty(&cq->ready_queue)) {
      struct queue_message *qm =
          list_first_entry(&cq->ready_queue, struct queue_message, node);
  ...
  for (;;) {
    ...
    if (waketime != PR_NOW || buflen + MESSAGE_MAX > sizeof(buf)) {
      if (buflen) {
        ...
        // 更新空闲时间,考虑USB-CDC帧时间
        double idletime =
            (eventtime > sq->idle_time ? eventtime : sq->idle_time);
        if (sq->serial_fd_type == SQT_USB_CDC)
          sq->idle_time =
              idletime + SQT_USB_CDC_FRAME_TIME; // 增加1ms USB帧时间
        else
...
struct serialqueue *__visible serialqueue_alloc(const char *port, void *params,
                                                char serial_fd_type,
                                                int client_id,
  ...
  pollreactor_add_fd(sq->pr, SQPF_SERIAL, sq->serial_fd, input_event,
                     sq->serial_fd_type == SQT_DEBUGFILE);

  // Reactor setup
  pollreactor_add_fd(sq->pr, SQPF_PIPE, sq->pipe_fds[0], kick_event, 0);
  pollreactor_add_timer(
      sq->pr, SQPT_RETRANSMIT,
      retransmit_event); //---3task-G-G--RW_task- //---send-G-G-2022-07-22---
  pollreactor_add_timer(sq->pr, SQPT_COMMAND,
                        command_event); //---send-G-G-2022-07-22---
...
void __visible serialqueue_free(struct serialqueue *sq) {
  ...
  while (!list_empty(&sq->pending_queues)) {
    struct command_queue *cq =
        list_first_entry(&sq->pending_queues, struct command_queue, node);
    list_del(&cq->node);
...
struct command_queue *__visible serialqueue_alloc_commandqueue(void) {
...
void serialqueue_send_batch(
    struct serialqueue *sq, struct command_queue *cq,
    struct list_head *
        msgs) //---29-2task-G-G--UI_control_task-- //-5-5task-G-G---get_clock---
{
  int len = 0;
  struct queue_message *qm;
...
void __visible serialqueue_send(struct serialqueue *sq,
                                struct command_queue *cq, uint8_t *msg, int len,
...
// available)
void __visible serialqueue_pull(struct serialqueue *sq,
  ...
  struct queue_message *qm =
      list_first_entry(&sq->receive_queue, struct queue_message, node);
  list_del(&qm->node);
...
void __visible serialqueue_set_wire_frequency(struct serialqueue *sq,
                                              double frequency) {
  // 设置调整后的速率
  pthread_mutex_lock(&sq->lock);
  if (sq->serial_fd_type == SQT_CAN) {
...
void __visible serialqueue_set_receive_window(struct serialqueue *sq,
                                              int receive_window) {
  pthread_mutex_lock(&sq->lock);
...
// serial port
void __visible serialqueue_set_clock_est(struct serialqueue *sq,
                                         double est_freq, double conv_time,
                                         uint64_t conv_clock,
                                         uint64_t last_clock) {
  pthread_mutex_lock(&sq->lock);
...
void __visible serialqueue_get_stats(struct serialqueue *sq, char *buf,
                                     int len) {
  ...
  snprintf(buf, len,
           ...
           stats.bytes_write, stats.bytes_read, stats.bytes_retransmit,
           stats.bytes_invalid, (int)stats.send_seq, (int)stats.receive_seq,
           (int)stats.retransmit_seq, stats.srtt, stats.rttvar, stats.rto,
           stats.ready_bytes, stats.stalled_bytes);
}

// Extract old messages stored in the debug queues
int __visible serialqueue_extract_old(struct serialqueue *sq, int sentq,
  ...
  while (!list_empty(&current)) {
    struct queue_message *qm =
        list_first_entry(&current, struct queue_message, node);
    if (qm->len && pos < max) {
